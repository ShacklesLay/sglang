...
class ForwardBatch:
    ...
    def init_new(
        ...
        # Extract and concatenate cross_attention_mask and vision_position_ids from multimodal_inputs list
        if batch.multimodal_inputs is not None:
            cross_attention_masks = []
            vision_position_ids_list = []
            
            for idx, mm_input in enumerate(batch.multimodal_inputs):
                if mm_input is not None:
                    if ret.forward_mode.is_extend() and hasattr(mm_input, 'cross_attention_mask') and mm_input.cross_attention_mask is not None:
                        encoder_len = batch.encoder_lens[idx]
                        extend_seq_len = batch.extend_seq_lens[idx]
                        cross_attention_masks.append(mm_input.cross_attention_mask[-encoder_len*extend_seq_len:])

                    if hasattr(mm_input, 'vision_position_ids') and mm_input.vision_position_ids is not None:
                        vision_position_ids_list.append(mm_input.vision_position_ids)
            
            # Concatenate and move to device
            if cross_attention_masks:
                ret.custom_mask = torch.cat(cross_attention_masks, dim=0).to(device, non_blocking=True)
                
            if vision_position_ids_list:
                ret.vision_position_ids = torch.cat(vision_position_ids_list, dim=0).to(device, non_blocking=True)
